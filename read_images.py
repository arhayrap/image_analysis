...
def get_data():
    ...
    # data = {"image": data[0]["image"] + data[1]["image"] + data[2]["image"],
    #         "label": data[0]["label"] + data[1]["label"] + data[2]["label"]}
    #
    # data["image"] = shuffle(data["image"], random_state=0)
    ...
    data["image"] = np.array(data["image"]).reshape(int(len(data["image"])), lw, lw)
